...
def main() -> None:
    ...
    ingest.add_argument("--query-file", help="path to a JSON file holding one search or an array of searches (avoids shell quoting problems)")
    ...
    subparsers.add_parser("profile", help="show counts by entity type, specialty, city and run")
    ...
    if args.command == "init-db":
        ...
        records, truncated, per_search = [], [], []
        if args.fixture:
            records = json.loads(Path(args.fixture).read_text()).get("results", [])
            searches = 0
        else:
            queries = _queries(Path(args.query_file).read_text() if args.query_file else args.query)
            searches = len(queries)
            try:
                for query in queries:
                    found, was_truncated = fetch_records(query=query, delay=args.delay)
                    records.extend(found)
                    per_search.append({"query": query, "records": len(found), "truncated": was_truncated})
                    if not found:
                        logging.warning("Search returned 0 records (check the spelling of the criteria): %s", query)
                    if was_truncated:
        ...
        print(json.dumps({"run_id": run_id, "records": count, "searches": searches, "per_search": per_search, "truncated_searches": truncated}, indent=2))
        ...
        print(json.dumps(profile(connection), indent=2))
